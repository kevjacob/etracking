-- Run once in Supabase SQL Editor (online project) if Account Management fails with
-- "Could not find the function public.set_user_password_admin ... in the schema cache"
-- or similar errors for delete_app_user / update_app_user_admin / list_app_users.
--
-- After running, PostgREST reloads its schema cache automatically (NOTIFY below).

create extension if not exists pgcrypto with schema extensions;

-- List all app users (superuser only)
...
-- Update another user (superuser only)
...
-- Set another user's password (superuser only)
