-- Chat: one global room; any logged-in app user can read and send.
...
-- Anyone with anon key can read (app restricts to logged-in users via UI).
create policy "Allow anon to select chat_messages"
  on public.chat_messages for select to anon using (true);

-- Insert only via RPC so we verify username and set user_id/username from app_users.
...
-- Realtime so new messages appear live
