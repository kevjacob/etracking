...
import { fetchIDTs, insertIDT, updateIDT, deleteIDT, createIDTRow } from '../api/idt'
...
function createIDT(overrides = {}) {
  return createIDTRow(overrides)
}

export default function IDTTrackingPage() {
  ...
  const [idtsLoading, setIdtsLoading] = useState(true)
  const [addIDTFormOpen, setAddIDTFormOpen] = useState(false)
  const [addIDTForm, setAddIDTForm] = useState(emptyAddIdtForm)
  const [addIDTFormError, setAddIDTFormError] = useState('')
  const [addIDTConfirmOpen, setAddIDTConfirmOpen] = useState(false)
  const [addIDTSubmitting, setAddIDTSubmitting] = useState(false)
  ...
  const [idtDatePickerRow, setIDTDatePickerRow] = useState(null)
  ...
  const [idtSearchQuery, setIdtSearchQuery] = useState('')
  const getIdtDate = useCallback((row) => row.idtDate, [])
  const getIdtSearch = useCallback((row) => row.idtNo, [])
  const sortIdts = useCallback((list) => sortBySerial(list, (row) => row.idtNo), [])
  const {
    availableMonths,
    selectedMonth,
    setSelectedMonth,
    filteredRows: filteredIDTs,
    pageRows: pagedIDTs,
    currentPage,
    totalPages,
    totalItems: filteredIdtCount,
    goToPage,
  } = useTrackingListView({
    pageKey: 'idt',
    rows: idts,
    searchQuery: idtSearchQuery,
    getDateField: getIdtDate,
    getSearchField: getIdtSearch,
    sortRows: sortIdts,
  })
  ...
  const loadIDTs = useCallback(async () => {
    setIdtsLoading(true)
    try {
      const data = await fetchIDTs()
      setIdts(Array.isArray(data) ? data : [])
    } catch (e) {
      console.error('Fetch IDT error:', e)
      setIdts([])
    }
    setIdtsLoading(false)
  }, [])
  ...
  const updateRow = (id, updates) => {
    ...
    setIdts((prev) => {
      ...
      updateIDT(id, row)
        .then((updated) => {
          if (updated) setIdts((p) => p.map((r) => (r.id === id ? updated : r)))
        })
        .catch((e) => console.error('Update IDT error:', e))
      return next
    })
  }
  ...
  const deleteRow = (id) => {
    ...
    deleteIDT(id)
      .then(() => setIdts((prev) => prev.filter((row) => row.id !== id)))
      .catch((e) => console.error('Delete IDT error:', e))
  }
  ...
  const handleDateChange = (rowId, value) => {
    ...
    const row = idts.find((r) => r.id === rowId)
  ...
  const handleIDTDateChange = (rowId, value) => {
    const parsed = parseDate(value)
    if (parsed) updateRow(rowId, { idtDate: parsed })
    setIDTDatePickerRow(null)
  }
  ...
  const handleDeliveryTimeAndAttachmentComplete = ({ slot, attachmentType: type }) => {
    ...
    setDeliveryModal({ open: false, rowId: null, dateLabel: '', skipTimeStep: false })
    if (type === 'none') return
    const idtRow = idts.find((r) => r.id === rowId)
  ...
  const handleInvoiceSearchConfirm = async () => {
    ...
    if (type === 'original' && selectedInvoice) {
      ...
      setInvoiceAttachedNotice({
        open: true,
        message: `Invoice ${invoiceNo} has been updated with IDT ${deliveryOrderRow.idtNo || forRowId}.`,
      })
    }
    if (type === 'copy' && selectedInvoice) {
      const row = idts.find((r) => r.id === forRowId)
  ...
  const applyChopSignRemarkUpdate = (pending, removeSelfCollect) => {
    ...
    const row = idts.find((r) => r.id === rowIdToUse)
    ...
    try {
      if (rowIdToUse) {
        ...
        if (fromDriver) {
          ...
          const leadRow = idts.find((r) => r.id === rowIdToUse)
  ...
  const setAddIDTFormField = (field, value) => {
    setAddIDTFormError('')
    if (field === 'idtDigits') {
      setAddIDTForm((prev) => ({ ...prev, idtDigits: normalizeDigits(value, IDT_DIGIT_LEN) }))
      return
    }
    if (field === 'withCharges') {
      setAddIDTForm((prev) => ({ ...prev, withCharges: !!value }))
      return
    }
    setAddIDTForm((prev) => ({ ...prev, [field]: value }))
  }

  const validateAddIDTForm = () => {
    const idtNo = formatIdtNo(addIDTForm.idtDigits)
    if (!idtNo) return 'IDT number must be exactly 10 digits.'
    if (!addIDTForm.idtDate) return 'IDT date is required.'
    if (!addIDTForm.fromWarehouseId) return 'Transfer from warehouse is required.'
    if (!addIDTForm.toWarehouseId) return 'Transfer to warehouse is required.'
    if (addIDTForm.fromWarehouseId === addIDTForm.toWarehouseId) {
      return 'Transfer from and transfer to must be different warehouses.'
    }
    if (idts.some((r) => (r.idtNo || '').trim().toUpperCase() === idtNo)) {
      return `IDT ${idtNo} already exists.`
    }
    return null
  }

  const getAddIDTPreview = () => {
    const idtNo = formatIdtNo(addIDTForm.idtDigits)
    const fromWh = warehouses.find((w) => w.id === addIDTForm.fromWarehouseId)
    const toWh = warehouses.find((w) => w.id === addIDTForm.toWarehouseId)
    const transfer =
      fromWh?.name && toWh?.name ? `${fromWh.name} > ${toWh.name}` : '–'
    return {
      idtNo,
      transfer,
      remark: idtNo
        ? buildIdtRemark(addIDTForm.withCharges, addIDTForm.remark)
        : '',
    }
  }

  const handleAddIDTProceed = () => {
    const err = validateAddIDTForm()
    if (err) {
      setAddIDTFormError(err)
      return
    }
    setAddIDTConfirmOpen(true)
  }

  const handleAddIDTConfirmYes = async () => {
    const err = validateAddIDTForm()
    if (err) {
      setAddIDTFormError(err)
      setAddIDTConfirmOpen(false)
      return
    }
    const idtNo = formatIdtNo(addIDTForm.idtDigits)
    const preview = getAddIDTPreview()
    setAddIDTSubmitting(true)
    try {
      const inserted = await insertIDT(
        createIDT({
          idtNo,
          idtDate: addIDTForm.idtDate,
          fromWarehouseId: addIDTForm.fromWarehouseId,
          toWarehouseId: addIDTForm.toWarehouseId,
          remark: preview.remark,
          discrepancy: saveAdditionalRemark(addIDTForm.additionalRemark),
      ...
      recordInitialStatus({
        ...
        documentNo: inserted.idtNo,
        status: inserted.status || 'Billed',
        statusAt: inserted.statusUpdatedAt,
      })
      setIdts((prev) => [...prev, inserted])
      setAddIDTFormOpen(false)
      setAddIDTConfirmOpen(false)
      setAddIDTForm(emptyAddIdtForm())
      setAddIDTFormError('')
    } catch (e) {
      console.error('Create IDT error:', e)
      setAddIDTFormError(e.message || 'Failed to create IDT.')
      setAddIDTConfirmOpen(false)
    } finally {
      setAddIDTSubmitting(false)
    }
  }

  const handleAddIDTConfirmNo = () => {
    setAddIDTConfirmOpen(false)
  }

  const handleAddIDTFormClose = () => {
    setAddIDTFormOpen(false)
    setAddIDTConfirmOpen(false)
    setAddIDTForm(emptyAddIdtForm())
    setAddIDTFormError('')
  ...
  const addIDTPreview = getAddIDTPreview()
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4 flex-wrap min-w-0">
          ...
          <label htmlFor="idt-no-search" className="text-sm font-medium text-slate-700 shrink-0">
            IDT No.
          </label>
          <input
            id="idt-no-search"
            type="text"
            value={idtSearchQuery}
            onChange={(e) => setIdtSearchQuery(e.target.value)}
            placeholder="Search by IDT no."
            ...
            aria-label="Search by IDT number"
          />
          </div>
          <TrackingMonthFilter
            id="idt-month-filter"
        ...
        <div className="flex items-center gap-4 shrink-0">
          <RefreshListButton onRefresh={loadIDTs} loading={idtsLoading} label="Refresh IDT list" />
        <button
          type="button"
          onClick={() => {
            setAddIDTFormOpen(true)
            setAddIDTConfirmOpen(false)
            setAddIDTForm(emptyAddIdtForm())
            setAddIDTFormError('')
          }}
      ...
      <div className="bg-white rounded-lg shadow border border-slate-200 overflow-x-auto">
        {idtsLoading ? (
          <div className="p-8 text-center text-slate-500">Loading IDT…</div>
        ) : filteredIdtCount === 0 ? (
          <div className="p-8 text-center text-slate-500">
            {idtSearchQuery.trim()
              ? 'No IDTs match your search.'
              : selectedMonth
                ? `No IDT for ${formatMonthLabel(selectedMonth)}.`
                : 'No IDT added yet. Click "Add New IDT" to add one.'}
          </div>
        ) : (
          ...
          <thead>
            <tr className="bg-slate-100 border-b border-slate-200">
              <th className="text-left py-3 px-4 font-semibold text-slate-700 w-12">
                <input
                  type="checkbox"
                  checked={filteredIDTs.length > 0 && selectedInvoiceIds.length === filteredIDTs.length}
                  onChange={(e) => {
                    if (e.target.checked) setSelectedInvoiceIds(filteredIDTs.map((r) => r.id))
                    else setSelectedInvoiceIds([])
                  }}
                  onClick={(e) => e.stopPropagation()}
                  className="rounded border-slate-300 text-blue-900 focus:ring-blue-900"
                  aria-label="Select all IDT"
                />
              </th>
              ...
              <th className="text-left py-3 px-4 font-semibold text-slate-700">IDT No</th>
              <th className="text-left py-3 px-4 font-semibold text-slate-700">Transfer</th>
              <th className="text-left py-3 px-4 font-semibold text-slate-700">IDT Date</th>
                ...
                >
                  <td className="py-2 px-4 w-12" onClick={(e) => e.stopPropagation()}>
                    <input
                      ...
                      onClick={(e) => e.stopPropagation()}
                      className="rounded border-slate-300 text-blue-900 focus:ring-blue-900"
                      aria-label={`Select IDT ${row.idtNo || row.id}`}
                    />
                  </td>
                  ...
                  <td className="py-2 px-4">
                    <span className="py-1.5 px-2 block text-slate-700">
                      {getIdtTransfer(row, warehouses) || '–'}
                    </span>
                  </td>
                  <td className="py-2 px-4">
                    <span className="py-1.5 px-2 block text-slate-700">
                      {row.idtDate ? formatDate(row.idtDate) : '–'}
                    </span>
                  </td>
                  <td className="py-2 px-4">
                    {isCompletedLocked ? (
        ...
        {!idtsLoading && filteredIdtCount > 0 && (
          <TrackingPagination
      ...
      {/* Add New IDT - Form */}
      {addIDTFormOpen && !addIDTConfirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] overflow-auto p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-slate-800 mb-4">Add New IDT</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">IDT Date</label>
                <input
                  type="date"
                  value={addIDTForm.idtDate}
                  onChange={(e) => setAddIDTFormField('idtDate', e.target.value)}
                  className="w-full py-2 px-3 border border-slate-300 rounded focus:ring-2 focus:ring-blue-900"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">IDT No.</label>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold text-slate-600 shrink-0">IDT</span>
                  <input
                    type="text"
                    inputMode="numeric"
                    maxLength={IDT_DIGIT_LEN}
                    value={addIDTForm.idtDigits}
                    onChange={(e) => setAddIDTFormField('idtDigits', e.target.value)}
                    className="flex-1 py-2 px-3 border border-slate-300 rounded focus:ring-2 focus:ring-blue-900 font-mono"
                  />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Transfer From</label>
                <select
                  value={addIDTForm.fromWarehouseId}
                  onChange={(e) => setAddIDTFormField('fromWarehouseId', e.target.value)}
                  className="w-full py-2 px-3 border border-slate-300 rounded focus:ring-2 focus:ring-blue-900"
                >
              ...
              <div>
                ...
                <select
                  value={addIDTForm.toWarehouseId}
                  onChange={(e) => setAddIDTFormField('toWarehouseId', e.target.value)}
                  className="w-full py-2 px-3 border border-slate-300 rounded focus:ring-2 focus:ring-blue-900"
                >
                  <option value="">Select warehouse</option>
                  {warehouses.map((w) => (
              ...
              <div>
                <label className="flex items-center gap-2 text-sm font-medium text-slate-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={addIDTForm.withCharges}
                    onChange={(e) => setAddIDTFormField('withCharges', e.target.checked)}
                    className="rounded border-slate-300 text-blue-900 focus:ring-blue-900"
                  />
                  With Charges
                </label>
                {!addIDTForm.withCharges && (
                  <p className="text-xs text-slate-500 mt-1">Unticked = Without Charges in remark</p>
                )}
              </div>
              <div>
                ...
                <input
                  type="text"
                  value={addIDTForm.remark}
                  onChange={(e) => setAddIDTFormField('remark', e.target.value)}
                  className="w-full py-2 px-3 border border-slate-300 rounded focus:ring-2 focus:ring-blue-900"
              ...
              <div>
                ...
                <input
                  type="text"
                  value={addIDTForm.additionalRemark}
                  onChange={(e) => setAddIDTFormField('additionalRemark', e.target.value)}
                  className="w-full py-2 px-3 border border-slate-300 rounded focus:ring-2 focus:ring-blue-900"
                  placeholder="Optional"
                />
              </div>
            ...
            {addIDTFormError && (
              <p className="mt-4 text-sm text-red-600 bg-red-50 border border-red-100 rounded-lg px-3 py-2">{addIDTFormError}</p>
            )}
            <div className="flex justify-end gap-2 mt-6">
              <button type="button" onClick={handleAddIDTFormClose} className="px-4 py-2 border border-slate-300 rounded-lg hover:bg-slate-50">Cancel</button>
              <button type="button" onClick={handleAddIDTProceed} className="px-4 py-2 bg-blue-900 text-white rounded-lg hover:bg-blue-800">Done</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Add New IDT - Confirm */}
      {addIDTFormOpen && addIDTConfirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={handleAddIDTConfirmNo}>
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-auto p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-slate-800 mb-2">Confirm new IDT</h3>
            <dl className="border border-slate-200 rounded-lg divide-y divide-slate-200 mb-6 text-sm">
              <div className="flex justify-between py-2 px-3"><dt className="text-slate-500">IDT No</dt><dd className="font-medium">{addIDTPreview.idtNo}</dd></div>
              <div className="flex justify-between py-2 px-3"><dt className="text-slate-500">IDT Date</dt><dd>{addIDTForm.idtDate ? formatDate(addIDTForm.idtDate) : '–'}</dd></div>
              <div className="flex justify-between py-2 px-3"><dt className="text-slate-500">Transfer</dt><dd>{addIDTPreview.transfer}</dd></div>
              <div className="flex justify-between py-2 px-3"><dt className="text-slate-500">Charges</dt><dd>{addIDTForm.withCharges ? 'With Charges' : 'Without Charges'}</dd></div>
              <div className="py-2 px-3"><dt className="text-slate-500 mb-1">Remark</dt><dd className="text-slate-700">{addIDTForm.remark?.trim() || '–'}</dd></div>
              <div className="py-2 px-3"><dt className="text-slate-500 mb-1">Additional Remark</dt><dd className="text-slate-700">{addIDTForm.additionalRemark?.trim() || '–'}</dd></div>
              <div className="py-2 px-3"><dt className="text-slate-500 mb-1">Saved remark</dt><dd className="text-slate-700">{addIDTPreview.remark}</dd></div>
            </dl>
            {addIDTFormError && (
              <p className="mb-4 text-sm text-red-600 bg-red-50 border border-red-100 rounded-lg px-3 py-2">{addIDTFormError}</p>
            )}
            <div className="flex justify-end gap-2">
              <button type="button" onClick={handleAddIDTConfirmNo} disabled={addIDTSubmitting} className="px-4 py-2 border border-slate-300 rounded-lg hover:bg-slate-50 disabled:opacity-50">No</button>
              <button type="button" onClick={handleAddIDTConfirmYes} disabled={addIDTSubmitting} className="px-4 py-2 bg-blue-900 text-white rounded-lg hover:bg-blue-800 disabled:opacity-50">
                {addIDTSubmitting ? 'Creating…' : 'Yes'}
              </button>
            </div>
          </div>
        </div>
      ...
      {cancelledConfirmModal.open && (() => {
        const row = idts.find((r) => r.id === cancelledConfirmModal.rowId)
        const serial = row?.idtNo || cancelledConfirmModal.rowId || 'this document'
        return (
      ...
      {bulkApplyConfirmModal.open && bulkApplyConfirmModal.rowIds.length > 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={handleBulkApplyNo}>
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-lg font-semibold text-slate-800 mb-3">Apply to selected IDT?</h3>
            ...
            <ul className="text-slate-700 text-sm mb-4 max-h-40 overflow-y-auto list-disc list-inside">
              {bulkApplyConfirmModal.rowIds.map((id) => {
                const inv = idts.find((r) => r.id === id)
                return (
                  <li key={id}>{inv?.idtNo || id}</li>
                )
              })}
            </ul>
