...
import { fetchGRCs, insertGRC, updateGRC, deleteGRC, createGRCRow } from '../api/grc'
import { fetchGRNs, insertGRN, updateGRN, createGRNRow } from '../api/grn'
...
import { buildInvoiceUpdateForDocLink } from '../utils/invoiceLinkSync'
...
function createGRC(overrides = {}) {
  return createGRCRow(overrides)
}

export default function GRCTrackingPage() {
  ...
  const [grcsLoading, setGrcsLoading] = useState(true)
  const [addGRCFormOpen, setAddGRCFormOpen] = useState(false)
  const [addGRCForm, setAddGRCForm] = useState(emptyAddGrcForm)
  const [addGRCFormError, setAddGRCFormError] = useState('')
  const [addGRCConfirmOpen, setAddGRCConfirmOpen] = useState(false)
  const [addGRCSubmitting, setAddGRCSubmitting] = useState(false)
  ...
  const [grcDatePickerRow, setGRCDatePickerRow] = useState(null)
  ...
  const [grcSearchQuery, setGrcSearchQuery] = useState('')
  const getGrcDate = useCallback((row) => row.grcDate, [])
  const getGrcSearch = useCallback((row) => row.grcNo, [])
  const sortGrcs = useCallback((list) => sortBySerial(list, (row) => row.grcNo), [])
  const {
    availableMonths,
    selectedMonth,
    setSelectedMonth,
    filteredRows: filteredGRCs,
    pageRows: pagedGRCs,
    currentPage,
    totalPages,
    totalItems: filteredGrcCount,
    goToPage,
  } = useTrackingListView({
    pageKey: 'grc',
    rows: grcs,
    searchQuery: grcSearchQuery,
    getDateField: getGrcDate,
    getSearchField: getGrcSearch,
    sortRows: sortGrcs,
  })
  ...
  const loadGRCs = useCallback(async () => {
    ...
    try {
      ...
      console.error('Fetch GRC error:', e)
      setGrcs([])
    }
    setGrcsLoading(false)
  }, [])
  ...
  const loadGrns = useCallback(async () => {
    try {
      ...
      console.error('Fetch GRN for GRC link error:', e)
      setGrns([])
    }
  }, [])
  ...
  const syncGrcToLinkedGrn = useCallback(
    async (grcRow) => {
      if (syncGuardRef.current) return
      const grnRow = resolveLinkedGrn(grcRow, grns)
      if (!grnRow) return
      syncGuardRef.current = true
      try {
        const merged = mergeLinkedSync(grnRow, grcRow)
        const updated = await updateGRN(grnRow.id, merged)
        if (updated) setGrns((prev) => prev.map((r) => (r.id === updated.id ? updated : r)))
      } catch (e) {
        console.error('Sync GRC to GRN error:', e)
      } finally {
  ...
  const updateRow = (id, updates) => {
    ...
    setGrcs((prev) => {
      ...
      updateGRC(id, row)
        .then((updated) => {
            if (updated) {
            setGrcs((p) => p.map((r) => (r.id === id ? updated : r)))
            if (hasLinkedGrn(updated, grns)) syncGrcToLinkedGrn(updated)
          }
        })
        .catch((e) => console.error('Update GRC error:', e))
      return next
    })
  }
  ...
  const deleteRow = (id) => {
    ...
    deleteGRC(id)
      .then(() => setGrcs((prev) => prev.filter((row) => row.id !== id)))
      .catch((e) => console.error('Delete GRC error:', e))
  }
  ...
  const handleDateChange = (rowId, value) => {
    ...
    const row = grcs.find((r) => r.id === rowId)
  ...
  const handleGRCDateChange = (rowId, value) => {
    const parsed = parseDate(value)
    if (parsed) updateRow(rowId, { grcDate: parsed })
    setGRCDatePickerRow(null)
  }
  ...
  const handleDeliveryTimeAndAttachmentComplete = ({ slot, attachmentType: type }) => {
    ...
    setDeliveryModal({ open: false, rowId: null, dateLabel: '', skipTimeStep: false })
    if (type === 'none') return
    const grcRow = grcs.find((r) => r.id === rowId)
  ...
  const handleInvoiceSearchConfirm = async (addAnother = false) => {
    ...
    if (type === 'original') {
      ...
      setInvoiceAttachedNotice({
        open: true,
        message: `Invoice ${invoiceNos.join(', ')} has been updated with GRC ${deliveryOrderRow.grcNo || forRowId}.`,
      })
    }
    if (type === 'copy') {
      const row = grcs.find((r) => r.id === forRowId)
  ...
  const applyChopSignRemarkUpdate = (pending, removeSelfCollect) => {
    ...
    const row = grcs.find((r) => r.id === rowIdToUse)
    ...
    try {
      if (rowIdToUse) {
        ...
        if (fromDriver) {
          ...
          const leadRow = grcs.find((r) => r.id === rowIdToUse)
  ...
  const setAddGRCFormField = (field, value) => {
    setAddGRCFormError('')
    if (field === 'grcDigits') {
      setAddGRCForm((prev) => ({ ...prev, grcDigits: normalizeDigits(value, GRC_DIGIT_LEN) }))
      return
    }
    if (field === 'doDigits') {
      setAddGRCForm((prev) => ({ ...prev, doDigits: normalizeDigits(value, DO_DIGIT_LEN) }))
      return
    }
    setAddGRCForm((prev) => ({ ...prev, [field]: value }))
  }

  const validateAddGRCForm = () => {
    const grcNo = formatGrcNo(addGRCForm.grcDigits)
    const doNo = formatDoNo(addGRCForm.doDigits)
    if (!grcNo) return 'GRC number must be exactly 10 digits.'
    if (!doNo) return 'GRN number must be exactly 5 digits.'
    if (!addGRCForm.doDate) return 'GRN date is required.'
    if (!addGRCForm.grcDate) return 'GRC date is required.'
    if (!(addGRCForm.outlet || '').trim()) return 'Outlet is required.'
    if (grcs.some((r) => (r.grcNo || '').trim().toUpperCase() === grcNo)) {
      return `GRC ${grcNo} already exists.`
    }
    if (grns.some((r) => (r.grnNo || '').trim().toUpperCase() === doNo)) {
      return `GRN ${doNo} already exists on GRN tracking.`
    }
    return null
  }

  const getAddGRCPreview = () => {
    const grcNo = formatGrcNo(addGRCForm.grcDigits)
    const doNo = formatDoNo(addGRCForm.doDigits)
    return {
      grcNo,
      ...
      grcRemark:
        grcNo && doNo
          ? buildGrcRemark(doNo, addGRCForm.doDate, addGRCForm.additionalRemark)
          : '',
      grnRemark: grcNo
        ? buildGrnRemarkFromGrc(grcNo, addGRCForm.outlet, addGRCForm.additionalRemark)
        : '',
    }
  }

  const handleAddGRCProceed = () => {
    const err = validateAddGRCForm()
    if (err) {
      setAddGRCFormError(err)
      return
    }
    setAddGRCConfirmOpen(true)
  }

  const handleAddGRCConfirmYes = async () => {
    const err = validateAddGRCForm()
    if (err) {
      setAddGRCFormError(err)
      setAddGRCConfirmOpen(false)
      return
    }
    const grcNo = formatGrcNo(addGRCForm.grcDigits)
    const doNo = formatDoNo(addGRCForm.doDigits)
    const preview = getAddGRCPreview()
    setAddGRCSubmitting(true)
    try {
      const insertedGrn = await insertGRN(
        createGRNRow({
          grnNo: doNo,
          grnDate: addGRCForm.doDate,
          remark: preview.grnRemark,
          numberAndDateLocked: true,
        })
      )
      recordInitialStatus({
        entityType: 'grn',
        entityId: insertedGrn.id,
        documentNo: insertedGrn.grnNo,
        status: insertedGrn.status || 'Billed',
        statusAt: insertedGrn.statusUpdatedAt,
      })
      const insertedGrc = await insertGRC(
        createGRC({
          grcNo,
          grcDate: addGRCForm.grcDate,
          outlet: (addGRCForm.outlet || '').trim(),
          remark: preview.grcRemark,
          numberAndDateLocked: true,
        })
      )
      recordInitialStatus({
        entityType: 'grc',
        entityId: insertedGrc.id,
        documentNo: insertedGrc.grcNo,
        status: insertedGrc.status || 'Billed',
        statusAt: insertedGrc.statusUpdatedAt,
      })
      const attachmentInvoices = normalizeAttachmentInvoices(addGRCForm.attachmentInvoices)
      for (const inv of attachmentInvoices) {
        if (!inv?.invoiceNo) continue
        const payload = buildInvoiceUpdateForDocLink(inv, insertedGrc, insertedGrc.grcNo)
        const updateApi = inv._source === 'autocount' ? updateAutocountInvoice : updateInvoice
        await updateApi(inv.id, payload)
      }
      setGrcs((prev) => [...prev, { ...insertedGrc, linkedGrnId: insertedGrn.id }])
      setGrns((prev) => [...prev, { ...insertedGrn, linkedGrcId: insertedGrc.id }])
      setAddGRCFormOpen(false)
      setAddGRCConfirmOpen(false)
      setAddGRCForm(emptyAddGrcForm())
      setAddGRCFormError('')
    } catch (e) {
      console.error('Create linked GRC/GRN error:', e)
      setAddGRCFormError(e.message || 'Failed to create GRC and linked GRN.')
      setAddGRCConfirmOpen(false)
    } finally {
      setAddGRCSubmitting(false)
    }
  }

  const handleAddGRCConfirmNo = () => {
    setAddGRCConfirmOpen(false)
  }

  const handleAddGRCFormClose = () => {
    setAddGRCFormOpen(false)
    setAddGRCConfirmOpen(false)
    setAddGRCForm(emptyAddGrcForm())
    setAddGRCFormError('')
  ...
  const addGRCPreview = getAddGRCPreview()
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4 flex-wrap min-w-0">
          ...
          <label htmlFor="grc-no-search" className="text-sm font-medium text-slate-700 shrink-0">
            GRC No.
          </label>
          <input
            id="grc-no-search"
            type="text"
            value={grcSearchQuery}
            onChange={(e) => setGrcSearchQuery(e.target.value)}
            placeholder="Search by GRC no."
            ...
            aria-label="Search by GRC number"
          />
          </div>
          <TrackingMonthFilter
            id="grc-month-filter"
        ...
        <div className="flex items-center gap-4 shrink-0">
          <RefreshListButton onRefresh={loadGRCs} loading={grcsLoading} label="Refresh GRC list" />
        <button
          type="button"
          onClick={() => {
            setAddGRCFormOpen(true)
            setAddGRCConfirmOpen(false)
            setAddGRCForm(emptyAddGrcForm())
            setAddGRCFormError('')
          }}
      ...
      <div className="bg-white rounded-lg shadow border border-slate-200 overflow-x-auto">
        {grcsLoading ? (
          <div className="p-8 text-center text-slate-500">Loading GRC…</div>
        ) : filteredGrcCount === 0 ? (
          <div className="p-8 text-center text-slate-500">
            {grcSearchQuery.trim()
              ? 'No GRCs match your search.'
              : selectedMonth
                ? `No GRC for ${formatMonthLabel(selectedMonth)}.`
                : 'No GRC added yet. Click "Add New GRC" to add one.'}
          </div>
        ) : (
          ...
          <thead>
            <tr className="bg-slate-100 border-b border-slate-200">
              <th className="text-left py-3 px-4 font-semibold text-slate-700 w-12">
                <input
                  type="checkbox"
                  checked={filteredGRCs.length > 0 && selectedInvoiceIds.length === filteredGRCs.length}
                  onChange={(e) => {
                    if (e.target.checked) setSelectedInvoiceIds(filteredGRCs.map((r) => r.id))
                    else setSelectedInvoiceIds([])
                  }}
                  onClick={(e) => e.stopPropagation()}
                  className="rounded border-slate-300 text-blue-900 focus:ring-blue-900"
                  aria-label="Select all GRC"
                />
              </th>
              ...
              <th className="text-left py-3 px-4 font-semibold text-slate-700">GRC No</th>
              <th className="text-left py-3 px-4 font-semibold text-slate-700">Outlet</th>
              <th className="text-left py-3 px-4 font-semibold text-slate-700">GRC Date</th>
                ...
                >
                  <td className="py-2 px-4 w-12" onClick={(e) => e.stopPropagation()}>
                    <input
                      ...
                      onClick={(e) => e.stopPropagation()}
                      className="rounded border-slate-300 text-blue-900 focus:ring-blue-900"
                      aria-label={`Select GRC ${row.grcNo || row.id}`}
                    />
                  </td>
                  ...
                  <td className="py-2 px-4">
                    <span className="py-1.5 px-2 block text-slate-700">
                      {row.grcDate ? formatDate(row.grcDate) : '–'}
                    </span>
                  </td>
                  <td className="py-2 px-4">
                    {isCompletedLocked ? (
        ...
        {!grcsLoading && filteredGrcCount > 0 && (
          <TrackingPagination
      ...
      {/* Add New GRC - Form */}
      {addGRCFormOpen && !addGRCConfirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] overflow-auto p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-slate-800 mb-4">Add New GRC</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">GRC Date</label>
                <input
                  type="date"
                  value={addGRCForm.grcDate}
                  onChange={(e) => setAddGRCFormField('grcDate', e.target.value)}
                  className="w-full py-2 px-3 border border-slate-300 rounded focus:ring-2 focus:ring-blue-900"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">GRC No.</label>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold text-slate-600 shrink-0">GRC</span>
                  <input
                    type="text"
                    inputMode="numeric"
                    maxLength={GRC_DIGIT_LEN}
                    value={addGRCForm.grcDigits}
                    onChange={(e) => setAddGRCFormField('grcDigits', e.target.value)}
              ...
              <div>
                ...
                <input
                  type="text"
                  value={addGRCForm.outlet}
                  onChange={(e) => setAddGRCFormField('outlet', e.target.value)}
                  className="w-full py-2 px-3 border border-slate-300 rounded focus:ring-2 focus:ring-blue-900"
              ...
              <div>
                ...
                <input
                  type="date"
                  value={addGRCForm.doDate}
                  onChange={(e) => setAddGRCFormField('doDate', e.target.value)}
                  className="w-full py-2 px-3 border border-slate-300 rounded focus:ring-2 focus:ring-blue-900"
                />
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    value={addGRCForm.doDigits}
                    onChange={(e) => setAddGRCFormField('doDigits', e.target.value)}
                    className="flex-1 py-2 px-3 border border-slate-300 rounded focus:ring-2 focus:ring-blue-900 font-mono"
                  />
                </div>
              </div>
              <div>
                ...
                <input
                  type="text"
                  value={addGRCForm.additionalRemark}
                  onChange={(e) => setAddGRCFormField('additionalRemark', e.target.value)}
                  className="w-full py-2 px-3 border border-slate-300 rounded focus:ring-2 focus:ring-blue-900"
              ...
              <InvoiceAttachmentSearch
                ...
                query={addGRCForm.attachmentQuery || ''}
                onQueryChange={(value) => setAddGRCFormField('attachmentQuery', value)}
                selected={addGRCForm.attachmentInvoices || []}
                onSelect={(invs) => setAddGRCFormField('attachmentInvoices', invs || [])}
                multiple
              />
            </div>
            {addGRCFormError && (
              <p className="mt-4 text-sm text-red-600 bg-red-50 border border-red-100 rounded-lg px-3 py-2">{addGRCFormError}</p>
            )}
            <div className="flex justify-end gap-2 mt-6">
              <button type="button" onClick={handleAddGRCFormClose} className="px-4 py-2 border border-slate-300 rounded-lg hover:bg-slate-50">Cancel</button>
              <button type="button" onClick={handleAddGRCProceed} className="px-4 py-2 bg-blue-900 text-white rounded-lg hover:bg-blue-800">Done</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Add New GRC - Confirm */}
      {addGRCFormOpen && addGRCConfirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={handleAddGRCConfirmNo}>
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-auto p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-slate-800 mb-2">Confirm new GRC</h3>
            <dl className="border border-slate-200 rounded-lg divide-y divide-slate-200 mb-6 text-sm">
              <div className="flex justify-between py-2 px-3"><dt className="text-slate-500">GRC No</dt><dd className="font-medium">{addGRCPreview.grcNo}</dd></div>
              <div className="flex justify-between py-2 px-3"><dt className="text-slate-500">GRC Date</dt><dd>{addGRCForm.grcDate ? formatDate(addGRCForm.grcDate) : '–'}</dd></div>
              <div className="flex justify-between py-2 px-3"><dt className="text-slate-500">GRN No</dt><dd className="font-medium">{addGRCPreview.doNo}</dd></div>
              <div className="flex justify-between py-2 px-3"><dt className="text-slate-500">GRN Date</dt><dd>{addGRCForm.doDate ? formatDate(addGRCForm.doDate) : '–'}</dd></div>
              <div className="flex justify-between py-2 px-3"><dt className="text-slate-500">Outlet</dt><dd>{addGRCForm.outlet || '–'}</dd></div>
              <div className="py-2 px-3"><dt className="text-slate-500 mb-1">Additional Remark</dt><dd className="text-slate-700">{addGRCForm.additionalRemark?.trim() || '–'}</dd></div>
              <div className="py-2 px-3"><dt className="text-slate-500 mb-1">GRC remark</dt><dd className="text-slate-700">{addGRCPreview.grcRemark}</dd></div>
              <div className="py-2 px-3"><dt className="text-slate-500 mb-1">GRN remark</dt><dd className="text-slate-700">{addGRCPreview.grnRemark}</dd></div>
            </dl>
            {addGRCFormError && (
              <p className="mb-4 text-sm text-red-600 bg-red-50 border border-red-100 rounded-lg px-3 py-2">{addGRCFormError}</p>
            )}
            <div className="flex justify-end gap-2">
              <button type="button" onClick={handleAddGRCConfirmNo} disabled={addGRCSubmitting} className="px-4 py-2 border border-slate-300 rounded-lg hover:bg-slate-50 disabled:opacity-50">No</button>
              <button type="button" onClick={handleAddGRCConfirmYes} disabled={addGRCSubmitting} className="px-4 py-2 bg-blue-900 text-white rounded-lg hover:bg-blue-800 disabled:opacity-50">
                {addGRCSubmitting ? 'Creating…' : 'Yes'}
              </button>
            </div>
          </div>
        </div>
      ...
      {cancelledConfirmModal.open && (() => {
        const row = grcs.find((r) => r.id === cancelledConfirmModal.rowId)
        const serial = row?.grcNo || cancelledConfirmModal.rowId || 'this document'
        return (
      ...
      {bulkApplyConfirmModal.open && bulkApplyConfirmModal.rowIds.length > 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={handleBulkApplyNo}>
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-lg font-semibold text-slate-800 mb-3">Apply to selected GRC?</h3>
            ...
            <ul className="text-slate-700 text-sm mb-4 max-h-40 overflow-y-auto list-disc list-inside">
              {bulkApplyConfirmModal.rowIds.map((id) => {
                const inv = grcs.find((r) => r.id === id)
                return (
                  <li key={id}>{inv?.grcNo || id}</li>
                )
              })}
            </ul>
