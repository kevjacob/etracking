/**
 * Autocount Invoices. Uses Supabase when configured, else localStorage.
 */
import { supabase, isSupabaseConfigured } from '../supabaseClient'
import { toSnakeCase, fromSnakeCase } from '../lib/dbMappers'

const STORAGE_KEY = 'etracking_invoices_autocount'
...
function saveInvoices(invoices) {
  try {
    ...
    console.error('Failed to save Autocount invoices to localStorage:', e)
  }
}

export async function fetchInvoices() {
  if (isSupabaseConfigured()) {
    const { data, error } = await supabase.from('invoices_autocount').select('*').order('created_at', { ascending: true })
...
export async function updateInvoice(id, row) {
  if (isSupabaseConfigured()) {
    ...
    const { data, error } = await supabase.from('invoices_autocount').update(payload).eq('id', id).select('*').single()
    if (error) throw error
    return fromSnakeCase(data)
  }
  const invoices = loadInvoices()
...
export async function deleteInvoice(id) {
  if (isSupabaseConfigured()) {
    const { error } = await supabase.from('invoices_autocount').delete().eq('id', id)
    if (error) throw error
    return
  }
