import { useMemo } from 'react'
import { searchInvoicesCombined } from '../utils/invoiceLinkSync'

function invoiceKey(inv) {
  if (!inv) return ''
  return `${inv._source || 'esd'}:${inv.id}`
}

export function normalizeAttachmentInvoices(selected) {
  if (!selected) return []
  return Array.isArray(selected) ? selected.filter(Boolean) : [selected]
}

export default function InvoiceAttachmentSearch({
  esdInvoices,
  autocountInvoices,
  query,
  onQueryChange,
  selected,
  onSelect,
  multiple = false,
}) {
  const q = String(query || '').trim()
  const selectedList = normalizeAttachmentInvoices(selected)

  const results = useMemo(() => {
    if (!q) return []
    return searchInvoicesCombined(esdInvoices, autocountInvoices, query)
  }, [esdInvoices, autocountInvoices, query, q])

  const isSelected = (inv) =>
    selectedList.some((s) => invoiceKey(s) === invoiceKey(inv))

  const toggle = (inv) => {
    if (!multiple) {
      onSelect(isSelected(inv) ? null : inv)
      return
    }
    if (isSelected(inv)) {
      onSelect(selectedList.filter((s) => invoiceKey(s) !== invoiceKey(inv)))
    } else {
      onSelect([...selectedList, inv])
    }
  }

  return (
    <div className="space-y-2">
      <label className="block text-sm font-medium text-slate-700">
        Attachment (Optional)
      </label>
      <input
        type="text"
        value={query}
        onChange={(e) => onQueryChange(e.target.value)}
        placeholder="Search ESD or Autocount invoice no."
        className="w-full py-2 px-3 border border-slate-300 rounded focus:ring-2 focus:ring-blue-900 text-sm"
      />
      {selectedList.length > 0 && (
        <ul className="space-y-1">
          {selectedList.map((inv) => (
            <li
              key={invoiceKey(inv)}
              className="text-sm text-blue-900 bg-blue-50 border border-blue-100 rounded px-3 py-2 flex items-center justify-between gap-2"
            >
              <span>
                Selected: <span className="font-medium">{inv.invoiceNo}</span>
                <span className="text-slate-500 ml-2 text-xs">
                  {inv._source === 'autocount' ? 'Autocount' : 'ESD'}
                </span>
              </span>
              <button
                type="button"
                onClick={() => toggle(inv)}
                className="text-slate-500 hover:text-slate-700 underline text-xs shrink-0"
              >
                Remove
              </button>
            </li>
          ))}
        </ul>
      )}
      {q && results.length === 0 && (
        <p className="text-sm text-amber-700 bg-amber-50 border border-amber-100 rounded px-3 py-2">
          No record found
        </p>
      )}
      {q && results.length > 0 && (
        <ul className="max-h-40 overflow-y-auto border border-slate-200 rounded divide-y divide-slate-100">
          {results.map((inv) => {
            const checked = isSelected(inv)
            return (
              <li key={invoiceKey(inv)}>
                <button
                  type="button"
                  onClick={() => toggle(inv)}
                  className={`w-full text-left px-3 py-2 text-sm hover:bg-slate-50 ${
                    checked ? 'bg-blue-50 text-blue-900 font-medium' : 'text-slate-700'
                  }`}
                >
                  {multiple && (
                    <span className="mr-2">{checked ? '☑' : '☐'}</span>
                  )}
                  {inv.invoiceNo}
                  <span className="text-slate-400 ml-2 text-xs">
                    {inv._source === 'autocount' ? 'Autocount' : 'ESD'}
                  </span>
                </button>
              </li>
            )
          })}
        </ul>
      )}
    </div>
  )
}
